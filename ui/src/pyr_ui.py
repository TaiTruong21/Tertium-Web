...
def main_page(req):
  return render_to_response('show_login.html', {}, request=req)
...
def post_login(req):
  if is_valid_user(req) == True:
    ...
    return render_to_response('show_login.html', {'error':'Invalid Credentials'}, request=req)
...
def get_signup_customer(req):
    return render_to_response('show_signup.html',{}, request=req)
